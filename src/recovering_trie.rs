...
impl RecoveringTrie {
  ...
  fn _verify_branch(&self, branch: BranchNode, ret: & mut HashMap<B256, B256>, walked_path: &Nibbles) -> Result<(), ThreadSafeError> {
      ...
      for index in CHILD_INDEX_RANGE {
          if !branch.state_mask.is_bit_set(index) { continue; }
  ...
  fn _verify_recursive_bytes(&self, 
      current: &mut &[u8],
  ...
  fn _verify_recursive_node(&self, 
      node: &TrieNode, 
      ret: & mut HashMap<B256, B256>,
      walked_path: Nibbles,    
  ) -> Result<(), ThreadSafeError> {
      ...
      match node {
          ...
          TrieNode::EmptyRoot => { bail_error!("Empty root node, verify fails (TODO: it should not necessarily fail_"); },
      }
      Ok(())
  }
              ...
              TrieNode::Extension(child_extension) => {
                  match TrieNode::decode(&mut &child_extension.child[..])? {
                      TrieNode::Branch(extension_child_branch) => {
                          return reconstruct_step_branch_node(
  ...
  for node in storage_proof.proof {
      ...
      match current_node {
          TrieNode::Branch(ref branch) => {
              reconstruct_step_branch_node(&branch, &current_tree)?
          }
          TrieNode::Extension(ref extension) => {
              Some(extension.child.to_vec())
          }
          TrieNode::Leaf(ref _leaf) => {
              None
          }
          TrieNode::EmptyRoot => { bail_error!("Empty root node in proof"); }
      };
