...
//
pub fn get_checkpoints_for_tag(tag: &str) -> Result<Vec<CheckPoint>, ThreadSafeError> {
...
pub fn get_specific_checkpoint(tag: &str, block: u64) -> Result<CheckPoint, ThreadSafeError> {
...
pub fn get_most_recent_checkpoint(tag: &str, block: u64) -> Result<CheckPoint, ThreadSafeError> {
  tprintln!("Getting checkpoint <= {} for {}", block, tag);
  let cpts = get_checkpoints_for_tag(tag)?;
  let cpts = cpts.iter().filter(|cpt| cpt.block <= block).collect::<Vec<&CheckPoint>>();
