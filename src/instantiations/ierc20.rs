...
use crate::{eventlogs::{apply_to_logs, LogEvent}, restore_contract_certain::{apply_updates, build_initial_storage_trie, simplify_updates, verify_memory_incremental}, storage_utils::{fetch_address_values, get_state_root, get_storage_root}, utils::{ProviderTrait, ThreadSafeError}, CHECK};
  ...
  INSTANCE: IERC20MemoryHandlerCertain + Send
{
  async fn initialize<T: ProviderTrait>(
      ...
      match self.load_state(current_block) {
        Ok(_) => {
          ...
          let initial_storage_root = get_storage_root(provider.clone(), BlockId::from(self.current_block()), initial_state_root, Self::contract_address()).await?;
  ...
  async fn update<T: ProviderTrait>(
      ...
      let state_updates = Arc::new(Mutex::new(INSTANCE::StateType::new()));
      {
        ...
        apply_to_logs(&*provider, Self::contract_address(), self.current_block()+1, target_block, move |logs| {
      ...
      {
        let hm_inter = fetch_address_values(provider.clone(), Self::contract_address(), block_id, storage_root, &addresses).await?;
