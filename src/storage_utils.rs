...
use alloy_rpc_types::{BlockId, serde_helpers::JsonStorageKey};
...
pub async fn get_state_root<T: ProviderTrait>(
  ...
  let block: alloy_rpc_types::Block = provider.block_by_number(block_number.into(), false).await?.ok_or("error")?;  
...
pub async fn get_account_info<T: ProviderTrait>(
  ...
  let storage_value  = provider.get_proof(contract_address, addreses.clone(), Some(block_number)).await?;
...
pub fn verify_storage_proof(
  ...
  storage_proof.iter().try_for_each(|proof: &EIP1186StorageProof| -> Result<(), ThreadSafeError> {
      let key = proof.key;
      let keyb = key.as_b256();
      let value = if proof.value.is_zero() { None } else { Some(encode_fixed_size(&proof.value).to_vec()) };
...
pub async fn get_storage_proof<T: ProviderTrait>(
  ...
  while i < addresses.len() {
      ...
      {
          let storage_proof = provider.get_proof(contract_address, addresses[i..i+chunk_size].to_vec(), Some(block_number)).await;
          match storage_proof {
              Ok(storage_proof) => {
  ...
  T: ProviderTrait + 'a
{
  ...
  tasks.for_each_concurrent(CONCURENCY_LIMIT, |res| async {
