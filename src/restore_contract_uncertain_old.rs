...
use alloy_rpc_types::{Block, Transaction, BlockId, serde_helpers::JsonStorageKey};
use reth_primitives::Receipt;
use reth_trie_common::proof::verify_proof;
use reth_trie_common::{Nibbles, TrieAccount};
use reth_primitives::revm_primitives::{keccak256, Address, B256};
use tokio::time::sleep;

use crate::recovering_trie::{RecoveringTrie, reconstruct_tree_from_verified_storage_proof};
...
async fn restore_contract_memory_with_contract_root<T: EthApiClient<Transaction, Block, Receipt> + Send + Sync>(
    provider: Arc<T>,
            ...
            loop {
                let ret;
                {
                    let _permit = semphore_clone.acquire().await;
                        ret = 
                    get_storage_proof(provider_clone.clone(), contract_address, block_number, adrv.clone()).await;
                }
    ...
    tasks.for_each_concurrent(CONCURENCY_LIMIT, |result| async {
            ...
            Ok((addresses, resp)) => {
                let res = task::spawn_blocking(move || {
                    tokio::runtime::Handle::current().block_on(async move {
                        let res = restore_contract_memory_with_contract_root_thread(resp, storage_root, addresses.to_vec(), rtclone, htclone).await;
                        res.unwrap();
                    })
                }).await;
                res.unwrap();
...
pub async fn restore_contract_memory<T: EthApiClient<Transaction, Block, Receipt> + Send + Sync>(
    provider: Arc<T>,
    contract_address: Address,
    block_number: BlockId,
    ...
    let locations = vec![JsonStorageKey( B256::from([0; 32]))];
    let storage_value  = provider.get_proof(contract_address, locations.clone(), Some(block_number)).await?;
    ...
    let (_rt, hm) = restore_contract_memory_with_contract_root(provider, contract_address, block_number, storage_root, addresses).await?;

    let mut addr_h = HashSet::new();
    for addr in addresses {
        let kek = keccak256(addr.0);
        addr_h.insert(kek);
    }
